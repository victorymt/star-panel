...
| 操作 | 触发 | 说明 |
|------|------|------|
| 切换显隐 | IPC `panel toggle` / Super+I | 由 Hyprland 绑定 |
| 关弹窗/关面板 | `Esc` / `Ctrl+Q` | 优先级：help/settings → edit/detail popup → 面板；输入框/搜索框聚焦时让它们自己处理 Esc |
...
底部输入栏左侧显示当前模式：`▸ NORMAL`（焦点在列表）/ `✎ INSERT`（输入中）/ `⌨ CMD`（命令模式）。

| 操作 | 触发 | 说明 |
|------|------|------|
| 切换类型 | `Tab` / `Shift+Tab` | 📋待办 → 💭灵感 → 📓日志 正/反向循环 |
| 进入命令模式 | `:` | 输入以 `:` 开头自动进入；支持模糊匹配（`:a` → `:idea`） |
...
- **快速输入**：在底部输入框输入内容，Enter 提交到 Starcatch；主面板切换 tab 时会同步提交类型，底部按 Tab 或点击类型按钮也会反向切换主面板
...
对应映射函数: `mapTodos()` / `mapIdeas()` / `mapLogs()`（`EntryMapper.js`）。
...
日志列表从 `starcatch log list -d N` 或 `starcatch log list --all` 输出解析，范围由“今天 / 近 3 天 / 近 7 天 / 近 30 天 / 全部”筛选项决定，默认并持久化为 3 天。切换范围时只重新获取日志，文本搜索继续作用于当前范围。
“全部”筛选要求使用支持 `log list --all` 的 Starcatch 版本；面板不会对旧版 CLI 做降级。
...
starcatch log list -d 3  # 面板会按当前筛选项传入 1 / 3 / 7 / 30，或使用 --all
```
