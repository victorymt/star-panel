...
| 📁 Todo 项目过滤 | 下拉按项目筛选待办，支持全部项目与未分类；选择会持久化 |
| 💭 灵感列表 | 标题 + 内容摘要 |
| 📓 日志列表 | 多行内容与图片缩略图，支持今天 / 近 3 天 / 近 7 天 / 近 30 天 / 全部筛选 |
...
待办列表顶部的项目下拉框会根据已加载的全部 Todo 动态生成选项。默认选择
**全部项目**；也可以选择具体项目，或选择 **未分类**（项目字段缺失、为空或为
`null` 的待办）。项目筛选会与状态筛选和关键词搜索同时生效，只有同时满足条件的
条目才会显示。切换 Pending / Done / Archived 不会清除项目选择。

项目筛选只在面板当前已加载的数据上执行，切换选项不会再次调用 Starcatch CLI。
当前选择会写入 `~/.config/star-panel/settings.json`，重启面板后自动恢复；已不存在的
项目会回退到“全部项目”。
